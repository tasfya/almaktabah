    ...
    describe '#attach_optimized_audio' do
      ...
      context 'when item responds to generate_bucket_key' do
        it 'uses custom bucket key with _op prefix' do
          ...
          allow(lecture).to receive(:respond_to?).with(:generate_bucket_key).and_return(true)
          allow(lecture).to receive(:generate_bucket_key).with(prefix: '_op').and_return('_op/custom/bucket/key.mp3')
          ...
          lecture.reload
          expect(lecture.optimized_audio).to be_attached
          expect(lecture).to have_received(:generate_bucket_key).with(prefix: '_op')
      ...
      context 'when item does not respond to generate_bucket_key' do
        it 'generates default bucket key' do
          allow(lecture).to receive(:respond_to?).and_call_original
          allow(lecture).to receive(:respond_to?).with(:generate_bucket_key).and_return(false)
